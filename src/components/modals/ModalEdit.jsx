        ...
        <div className="modal">
          <div className="modal__content">
            ...
            <form onSubmit={handleSubmit(updateOneBook)}>
              ...
              <label>Titulo</label>
              <input {...register('title')} type="text" value={bookExist.title}></input>
              <label>Número de Páginas</label>
              <input {...register('pages')} type="text" value={bookExist.pages}></input>
              {errors.pages && (<span>{errors.pages.message}</span>)}
              <label>ISBN</label>
              <input {...register('isbn')} type="text" value={bookExist.isbn}></input>
              {errors.isbn && (<span>{errors.isbn.message}</span>)}
              <label>Editora</label>
              <input {...register('publisher')} type="text" value={bookExist.publisher}></input>
              <label>Descrição</label>
              <textarea {...register('sumary')} type="text" rows={5} value={bookExist.sumary}></textarea>
